...
int decode(char *src_name, char *output_file_name){
    ...
    if(input_size > DECODE_WRITING_BUFF_SIZE)
        write_buffer_size = DECODE_WRITING_BUFF_SIZE;
    else
        write_buffer_size = input_size;
    buffer_write = (char *)malloc(sizeof(char) * write_buffer_size);
    while(1){
        fread_return = fread(buffer_read, sizeof(char), buffer_size, encoded_file);
        if(!fread_return){
            break;
        }
        for(int i = 0; i != fread_return; i++){
            ...
            for(int j = 0; j != BIT_SPACE; j++){
                ...
                if(writing_index == write_buffer_size){
                    fwrite(buffer_write, sizeof(char), writing_index * sizeof(char), file_after_decode);
                    writing_index = 0;
                }
                output_char <<= 1;
